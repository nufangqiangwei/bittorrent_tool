...
def decodeTorrentData(torrentData: bytes) -> dict:
    metadata = bencoder.decode(torrentData)
    ...
    def decodeBytes(data):
        ...
        if isinstance(data, list):
            return [decodeBytes(i) for i in data]
        ...
        if isinstance(data, dict):
            ...
            for k, v in data.items():
                result[decodeBytes(k)] = decodeBytes(v)
            return result
        raise Exception("data类型错误")

    result = {}
    for key, value in metadata.items():
        result[decodeBytes(key)] = decodeBytes(value)
    return result


if __name__ == '__main__':
    file_path = "E:\\GoCode\\testTorrent\\ubuntu-21.04-desktop-amd64.iso.torrent"
    file = open(file_path, 'rb')
    torrent = file.read()
    metadata = decodeTorrentData(torrent)
